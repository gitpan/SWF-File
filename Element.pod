...
represents a clip action triggered by clip event.
 ...
 field                     class
 EventFlags6               $            - SWF version>=6 only
   ClipEventKeyPress       (EventFlags6)
   ClipEventDragOut        (EventFlags6)
   ClipEventDragOver       (EventFlags6)
 EventFlags                $
...
enables debugging.
EnableDebugger is for SWF 5, and EnableDebugger2 is for SWF 6.
...
=item ACTIORECORD::ActionConstantPool
